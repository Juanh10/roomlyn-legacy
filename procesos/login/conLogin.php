...
if (!empty($_POST['usuario']) && !empty($_POST['contraseña'])) {
    ...
    $validar = $dbh -> prepare("SELECT usuarios.idUsuario, usuarios.tipoUsuario, usuarios.usuario, infousuarios.pNombre, infousuarios.pApellido FROM usuarios JOIN infousuarios ON usuarios.id_infoUsuario = infousuarios.id_infoUsuario WHERE usuarios.usuario = :usua and usuarios.contraseña = :contra  and usuarios.estado = :estado"); //* preparar la consulta

    //* Bloque para enlazar los marcadores con las variables
    $validar -> bindParam(':usua', $usuario);
    $validar -> bindParam(':contra', $contraseña);
