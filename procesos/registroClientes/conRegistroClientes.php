...
if (!empty($_POST['nombres']) && !empty($_POST['apellidos']) && !empty($_POST['documento']) && !empty($_POST['celular']) && !empty($_POST['email']) && !empty($_POST['sexo']) && !empty($_POST['nacionalidad']) && !empty($_POST['departamento']) && !empty($_POST['ciudad']) && $_POST['contrasena']) {
    ...
    // Preparar la consulta para insertar información del cliente
    ...
    // Asociar parámetros a la consulta de información del cliente
    ...
    // Preparar la consulta para verificar si el usuario ya está registrado
    ...
    if ($sqlConstUs->rowCount() > 0) {
        ...
        // Ejecutar la consulta para insertar información del cliente
        if ($sqlInforCliente->execute()) {
            // Obtener el último ID del cliente registrado
            $ultIDCliente = $dbh->lastInsertId('info_clientes');

            // Preparar la consulta para insertar el cliente registrado
            ...
            // Asociar parámetros a la consulta de cliente registrado
            ...
            // Ejecutar la consulta de cliente registrado
            if ($sqlCliente->execute()) {
                ...
                // Mostrar mensaje de error si hay un problema con la consulta de cliente registrado
            ...
            // Mostrar mensaje de error si hay un problema con la consulta de información del cliente
            $_SESSION['msjError'] = "Ha habido un error en el proceso. Por favor, te solicitamos amablemente que nos contactes mediante el correo electrónico hotelroomlyn@gmail.com para informarnos sobre este inconveniente.";
            header("location: ../../vistas/registroClientes.php");
            exit;
        }
    ...
    // Mostrar mensaje de error si hay campos vacíos en el formulario
